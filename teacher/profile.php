...
// require_once('../includes/teacher_sidebar_classes.php');
    ...
    if (isset($_POST['action'])) {
        if ($_POST['action'] == 'update_profile') {
            try {
                ...
                $avatar_path = $currentUser['avatar']; // Keep existing avatar by default
            ...
            if (!empty($_POST['new_password'])) {
                ...
                if (password_verify($_POST['current_password'], $current_hash)) {
                    $passwordStmt = $pdo->prepare("UPDATE teachers SET password = ? WHERE id = ?");
                    ...
                    try {
                        ...
                        $backupHooks->backupTeacherPasswordChange($_SESSION['user_id'], $updatedData);
                    } catch (Exception $e) {
...
$stmt = $pdo->prepare("
    ...
    JOIN subject_assignments sa ON sa.teacher_id = c.teacher_id AND sa.subject_id = c.subject_id AND sa.section_id = (SELECT id FROM sections WHERE name = c.section AND year_level = s.year_level LIMIT 1)
    ...
    <main class="page-content">
        ...
        <div class="form-card animate-fadeIn delay-1">
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-control bg-light" readonly><?php echo !empty($user['birth_date']) ? date('F d, Y', strtotime($user['birth_date'])) : 'N/A'; ?></div>
                </div>
                <div class="col-md-6">
                    <label class="form-label">Phone Number</label>
                    <div class="form-control bg-light" readonly><?php echo !empty($user['phone_number']) ? htmlspecialchars($user['phone_number']) : (!empty($user['phone']) ? htmlspecialchars($user['phone']) : 'N/A'); ?></div>
                </div>
                <div class="col-md-6">
    ...
    <script>
        ...
        function previewAvatar(input) {
            if (input.files && input.files[0]) {
                ...
                const avatarContainer = input.closest('.profile-avatar');
                const userAvatars = document.querySelectorAll('.user-avatar');
                reader.onload = function(e) {
                    // Update main profile avatar
                    let existingImg = avatarContainer.querySelector('.avatar-image');
                    if (!existingImg) {
                        existingImg = document.createElement('img');
                        existingImg.className = 'avatar-image';
                        // Remove only the initials/text nodes, not the edit icon or file input
                        avatarContainer.childNodes.forEach(node => {
                            if (node.nodeType === Node.TEXT_NODE) {
                                node.remove();
                            }
                        });
                        avatarContainer.insertBefore(existingImg, avatarContainer.firstChild);
                    }
